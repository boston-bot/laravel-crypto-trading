...
class SyncCoinbaseAssetsJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(
        ...
        foreach ($assets as $rawAsset) {
            $quoteCurrency = strtoupper((string) ($rawAsset['quote_currency_id'] ?? ''));
            if ($quoteCurrency !== strtoupper((string) config('broker.coinbase.quote_currency', 'USD'))) {
                continue;
            }
