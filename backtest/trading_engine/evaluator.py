...
class PortfolioEvaluator:
    ...
    def evaluate(self, feature_frames: Mapping[str, pd.DataFrame], assets: list[dict[str, Any]], as_of: datetime, portfolio_context: dict[str, Any], evidence_hash: str, states: Mapping[str, StrategyState] | None = None) -> dict[str, Any]:
        ...
        for asset in assets:
            ...
            proposals.append({"asset_id": int(asset["id"]), "asset": symbol, "action": action, "resolution": resolution, "decision_trace": trace, "portfolio_target": target, "order_intent": None, "warnings": reason if resolution == "blocked_by_evidence" else []})
