...
function renderOverview(data) {
    ...
    return `
        <section class="ops-hero-grid">
            ...
            <article class="ops-summary-card"><span>Strategy · paper</span><strong>${formatPct(data.paper?.return_pct)}</strong><p>${session ? `Session #${session.id} · ${session.funding_mode} capital` : 'No active paper session'}</p><a href="/paper">Open paper portfolio →</a></article>
...
function renderStrategies(data) {
    ...
    return `<section class="ops-feature-strip"><div><span class="ops-section-label">Active strategy</span><h2>${escapeHtml(data.active_name)}</h2><p>Closed 4-hour bars · spot long/flat · five-asset universe</p></div><div class="ops-feature-stat"><span>Versions</span><strong>${versions.length}</strong></div><div class="ops-feature-stat"><span>Backtests</span><strong>${tests.length}</strong></div></section>
...
function renderAssets(data) {
    ...
    return `<div class="ops-filter-row"><span>Comparing strategy contribution with Coinbase buy-and-hold</span><label>Window <select id="windowDays" data-action="window"><option value="7" ${data.window_days === 7 ? 'selected' : ''}>7 days</option><option value="30" ${data.window_days === 30 ? 'selected' : ''}>30 days</option><option value="90" ${data.window_days === 90 ? 'selected' : ''}>90 days</option></select></label></div>
        <section class="ops-asset-grid">${assets.map(asset => `<article class="ops-asset-card"><header><div><span class="ops-coin">${escapeHtml(asset.symbol.slice(0, 1))}</span><div><h2>${escapeHtml(asset.symbol)}</h2><p>${asset.position ? `${formatNumber(asset.position.quantity, 6)} held` : 'No open paper position'}</p></div></div>${statusTag(asset.latest_evaluation?.action || 'NOT EVALUATED')}</header><div class="ops-asset-comparison"><div><span>Paper contribution</span><strong class="${tone(asset.portfolio_contribution_pct)}">${formatPct(asset.portfolio_contribution_pct)}</strong><small>${formatMoney(asset.net_pnl)} net P&amp;L</small></div><div><span>Buy &amp; hold</span><strong>${formatPct(asset.benchmark_return_pct)}</strong><small>${asset.benchmark_available ? `${data.window_days}d Coinbase closes` : 'Boundary prices unavailable'}</small></div></div><div class="ops-evaluation-note"><span>Latest evaluation</span><p>${escapeHtml(asset.latest_evaluation?.primary_explanation || 'No evaluation has been recorded for this asset.')}</p></div></article>`).join('')}</section>`;
}
...
function renderPaper(data) {
    ...
    if (!data.session) return `<section class="ops-paper-onboarding"><div><span class="ops-section-label">Create a paper session</span><h2>Give the strategy capital it can actually account for.</h2><p>Virtual mode starts with an independent balance. Mirror mode converts the latest fresh Coinbase equity snapshot into paper USD cash without copying positions.</p></div><form id="paperSessionForm" class="ops-session-form"><input type="hidden" name="broker_account_id" value="${data.account.id}"><label>Funding mode<select name="funding_mode" id="fundingMode"><option value="virtual">Virtual capital</option><option value="mirror">Mirror Coinbase equity (${formatMoney(data.account.equity)})</option></select></label><label>Virtual starting capital<input name="virtual_capital" type="number" min="1" step="0.01" value="10000"></label><button class="ops-button primary" type="submit">Start paper session</button></form></section>${renderSessionHistory(data.history || [])}`;
    const s = data.session;
    return `<section class="ops-feature-strip"><div><span class="ops-section-label">Active session #${s.id}</span><h2>${formatMoney(data.snapshot?.equity ?? s.opening_cash)}</h2><p>${s.funding_mode} capital · started ${shortDate(s.started_at)} · ${escapeHtml(s.fee_scenario)}</p></div><div class="ops-feature-stat"><span>Available cash</span><strong>${formatMoney(data.available_cash)}</strong></div><div class="ops-feature-stat"><span>Drawdown</span><strong>${formatPct(data.snapshot?.drawdown_pct)}</strong></div><button class="ops-button danger" data-action="end-session" data-session="${s.id}">End session</button></section>
